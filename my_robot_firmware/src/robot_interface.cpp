#include "my_robot_firmware/robot_interface.hpp"
...
RobotInterface::~RobotInterface(){
    if(arduino_.IsOpen()){
        ...
        {
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("RobotInterface"), "Something went wrong while closing connection with port " << port_);
        }
...
hardware_interface::return_type RobotInterface::read(const rclcpp::Time &, const rclcpp::Duration &)
{
    if(arduino_.IsDataAvailable()){
        auto delta_time = (rclcpp::Clock().now() - last_run_).seconds();
        std::string message;
...
hardware_interface::return_type RobotInterface::write(const rclcpp::Time &, const rclcpp::Duration &)
{
    ...
    try{
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("RobotInterface"), "Something went wrong while sending the message" << message_stream.str() << " to the port " << port_);
...
PLUGINLIB_EXPORT_CLASS(my_robot_firmware::RobotInterface, hardware_interface::SystemInterface)